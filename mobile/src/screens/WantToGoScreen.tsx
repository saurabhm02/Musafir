...
function BookmarkPurpleIcon({ size = 20 }: { size?: number }) {
  return (
...
type Props = NativeStackScreenProps<RootStackParamList, "WantToGo">;

export function WantToGoScreen({ navigation }: Props) {
  ...
  const loadPlaces = useCallback(async () => {
    setLoading(true);
    try {
      const items = await fetchPoiStatusPlaces("want_to_go");
      setPlaces(items);
    } catch {
      setPlaces([]);
    } finally {
    ...
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
      ...
      {isSearchOpen && (
        <View style={styles.searchBarWrap}>
          <TextInput
            ...
            placeholder="Search wishlist places..."
      ...
      <View style={styles.subHeaderRow}>
        <Text style={styles.subHeaderText}>
          {places.length} place{places.length === 1 ? "" : "s"} in your wishlist
        </Text>
      </View>
      ...
      {loading ? (
        ...
        <View style={styles.emptyBox}>
          <View style={styles.emptyIconCircle}>
            <BookmarkPurpleIcon size={32} />
          </View>
          <Text style={styles.emptyTitle}>Wishlist is empty</Text>
          <Text style={styles.emptyDesc}>
            {searchQuery.trim()
              ? `No wishlist places matching "${searchQuery}".`
              : "Mark places you plan to visit to build your dream adventure itinerary."}
          </Text>
          <TouchableOpacity
          ...
          {filteredPlaces.map((p) => {
            ...
            return (
              ...
              >
                ...
                {/* Right Bookmark Icon (Tapping removes from Want to Go) */}
                <TouchableOpacity
                  style={styles.unsaveBtn}
                  onPress={() => handleRemove(p.id)}
                  hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
                >
                  <BookmarkPurpleIcon size={20} />
                </TouchableOpacity>
              </TouchableOpacity>
            );
          })}
